...
class HistoryWeatherRepositoryImpl:HistoryWeatherRepository {
    ...
    override fun fetchDailyHistoryWeather(
        ...
        return HistoryWeather(
            mutableListOf<Pair<String,DataWeather>>().apply {
                for(i in 0..(endYear.toInt()-startYear.toInt())){
                    for(month in 1..12){
                        fetchWeatherForMonth(indexCity,(startYear.toInt()+i).toString(),month.toString())?.let {
                            this.addAll(it)
                        }
                    }
                }
    ...
    private fun fetchWeatherForMonth(indexCity: String, year: String, month: String):List<Pair<String, DataWeather>>?{
        ...
        return if(tbodies.isNotEmpty()) {
            mutableListOf<Pair<String, DataWeather>>().apply {
                tbodies[1].select("tr").forEach {
                    ...
                    add(year+"-"+month+"-"+tds[0].text() to DataWeather(
